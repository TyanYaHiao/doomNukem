...
/*   Updated: 2019/12/07 17:38:17 by djast            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "editor.h"

static int	create_file(t_sdl *sdl, char *map_name)
{
	int		fd;

	ft_strcpy(sdl->filename, "game/resources/levels/");
	ft_strcat(sdl->filename, map_name);
	ft_strcat(sdl->filename, ".lvl");
	fd = open(sdl->filename, O_WRONLY);
	if (fd == -1)
	{
		open(sdl->filename, O_CREAT);
		fd = open(sdl->filename, O_WRONLY);
	}
	else
		return (-1);
