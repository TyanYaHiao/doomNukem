...
/*   Updated: 2019/12/21 12:05:35 by eharrag-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		write_crc(int fd)
{
	char			*buff;
	int				number;
	char			*crc;

	buff = (char *)malloc(sizeof(char) * BUFFMAX);
	number = read(fd, buff, BUFFMAX);
	buff[number] = '\0';
	crc = ft_itoa(calculate_crc(buff, number));
	free(buff);
	write(fd, "crc: ", 5);
